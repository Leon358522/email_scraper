...
WEBSITE_NAME = r'''((?<=\/\/)|(?<=\/\/www\.))[^(www)].+(?=\.)'''
logging.basicConfig(filename='emails.log', encoding='utf-8', level=logging.INFO, format='%(message)s')
EMAIL_REGEX = r"""(?:[a-z0-9!#$%&'*+\/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+\/=?^_`{|}~-]+)*|"(?:[
\x01-\x08\x0b\x0c\x0e-\x1f\x21\x23-\x5b\x5d-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])*")(@|\(at\))(?:(?:[a-z0-9](?:[a-z0-9-]*[
...
if __name__ == '__main__':
    ...
    for i, mails in enumerate(mails):
        ...
        try:
            var = {f'CompanySiteName: {re.search(WEBSITE_NAME, sites.__getitem__(i))[0].capitalize()}: {mails}'}
        except TypeError:
            continue
        print(var)
        logging.info(var)
